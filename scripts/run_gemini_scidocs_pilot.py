"""
run_gemini_scidocs_pilot.py
============================
Real Google Gemini pairwise pilot on SciDocs — 20 queries, top-k=20.

This script makes REAL Gemini API calls (no mock/dry-run).  Every judgment is
...
Outputs to: outputs/gemini_scidocs_real_pilot/
  - GEMINI_PILOT_SUMMARY.md
  - gemini_per_query.csv
  - gemini_summary.csv
  - judgments.jsonl
  - config.json
"""
...
def main():
    ...
    if not api_key:
        print("ERROR: GEMINI_API_KEY not set. This script requires real Gemini API calls.")
        sys.exit(1)
    ...
    print(f"\n{'=' * 70}")
    print("  REAL Gemini Pairwise Pilot — SciDocs")
    print(f"{'=' * 70}")
    ...
    print("  position debias  : DISABLED (single direction per pair)")
    print("  caching          : disk-backed, resumable")
    print(f"  output_dir       : {OUTPUT_DIR}")
    n_pairs_per_q = TOP_K * (TOP_K - 1) // 2
    total_api_calls_est = n_pairs_per_q * MAX_QUERIES
    print(f"  est. API calls   : {total_api_calls_est} "
          f"({n_pairs_per_q} pairs × {MAX_QUERIES} queries)")
    print()
    ...
    # ---- 4. Collect REAL Gemini pairwise judgments ----
    ...
    api_error_msg = None
    print("\n[4] Collecting REAL Gemini pairwise judgments …")
    ...
    all_rows: list[dict] = []
    print("\n[5] Running aggregation methods on real Gemini judgments …")
    ...
    pq_path = OUTPUT_DIR / "gemini_per_query.csv"
    if all_rows:
    ...
    summary_path = OUTPUT_DIR / "gemini_summary.csv"
    if summary_rows:
    ...
    label = (
        f"REAL GEMINI PILOT ({n_actual_queries} queries"
    ...
    md_lines = [
        ...
        "|-----------|-------|",
        f"| Dataset | {DATASET} |",
        f"| Queries | {n_actual_queries} (sampled from {len(eligible_qids)} eligible) |",
        f"| Top-k candidates | {TOP_K} |",
        f"| Seed | {SEED} |",
        f"| Model | {MODEL} |",
        f"| Provider | **{PROVIDER}** (Google Gemini) |",
        ...
        f"| Rate limit events | {len(rate_limit_events)} |",
    ]

    if is_partial:
        md_lines.extend([
            "",
            f"**Partial run** — stopped at API error: `{api_error_msg}`",
        ])
    ...
    md_lines.extend([
        ...
        "",
        "---",
        "*This report was generated from REAL Google Gemini API calls, "
        "not mock/synthetic judgments.*",
    ])

    md_path = OUTPUT_DIR / "GEMINI_PILOT_SUMMARY.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    print(f"[9] Comparison report → {md_path}")

    # ---- 10. Print to stdout ----
    print(f"\n{'=' * 70}")
    print("  REAL GEMINI PILOT COMPARISON TABLE — SciDocs")
