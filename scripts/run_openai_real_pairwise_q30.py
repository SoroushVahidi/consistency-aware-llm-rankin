"""
run_openai_real_pairwise_q30.py
===============================
Extend the existing real OpenAI SciDocs pairwise evidence from 20 to 30 queries
while reusing the previously collected cache.

Writes a fresh evidence package under:
  outputs/openai_scidocs_real_pairwise_q30_k15/
"""
...
MODEL = "gpt-4o-mini"
PROVIDER = "openai"
OUTPUT_DIR = Path("outputs/openai_scidocs_real_pairwise_q30_k15")
SOURCE_CACHE_DIR = Path("outputs/openai_scidocs_real_run_q20_k15/judgment_cache")
...
def main():
    ...
    if not SOURCE_CACHE_DIR.exists():
        print(f"ERROR: Source cache does not exist: {SOURCE_CACHE_DIR}")
        sys.exit(1)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=False)
    cache_dir = OUTPUT_DIR / "judgment_cache"
    shutil.copytree(SOURCE_CACHE_DIR, cache_dir)

    print(f"\n{'='*70}")
    print("  REAL OpenAI Pairwise Extension — SciDocs")
    print(f"{'='*70}")
    ...
    print(f"  source cache : {SOURCE_CACHE_DIR}")
    ...
    print("[1] Collecting / reusing pairwise judgments …")
    for idx, (qid, qt, cands) in enumerate(pool):
        t0 = time.time()
        try:
            pairs, _ = collect_all_pairs(qid, qt, cands, config=config, stats=stats)
        ...
        print(
            ...
            f"{len(pairs)} pairs {dt:.1f}s (API={stats.api_calls} cache={stats.cache_hits})"
        )
    wall_elapsed = time.time() - wall_start

    if not all_prefs:
        print("FATAL: No pairwise judgments available.")
        sys.exit(1)
    ...
    ss = stats.summary()
    total_cost = (
        ss["prompt_tokens"] / 1e6 * GPT4O_MINI_INPUT_COST_PER_M
        + ss["completion_tokens"] / 1e6 * GPT4O_MINI_OUTPUT_COST_PER_M
    )
    ...
    for qid, _, cands in pool:
        ...
        ref, rel = _ref_ranking(qr, set(graph.nodes()))
        ...
        for mn in METHOD_LABELS:
            ra = [d for d in rnk[mn] if d in set(ref)]
    ...
    for m in METHOD_LABELS:
        ...
        def _mn(k):
            vals = [r[k] for r in mr if r.get(k) is not None]
        ...
        srows.append(
            {
                ...
                "precision_mean": round(_mn("precision_at_k"), 4)
                if _mn("precision_at_k") is not None else None,
                "recall_mean": round(_mn("recall_at_k"), 4)
                if _mn("recall_at_k") is not None else None,
    ...
    cfg = {
        "label": f"REAL OPENAI RUN ({len(pool)} queries{', PARTIAL' if partial else ''}) — NOT mock",
        "dataset": DATASET,
    ...
    avg_fas = (
        sum(r["n_fas_removed"] for r in rows if r["method"] == "llm_pairwise_copeland") / tot_q
        if tot_q else 0.0
    )
    ...
    md += [
        ...
        "## Graph Stats",
        "",
        f"- Cyclic queries: {cyc_cnt}/{tot_q} ({cyc_cnt / tot_q * 100:.1f}%)" if tot_q else "- Cyclic queries: n/a",
        f"- Avg FAS edges removed: {avg_fas:.1f}",
        "",
        "---",
