...
METHODS = ("raw_score", "score_sum", "borda", "pagerank", "greedy_fas_topological")
...
def run_query(
    *,
    ...
    score_weight_scheme: str,
    scores_by_query: dict[str, list[tuple[str, float]]] | None,
    ...
    weight_scheme:
        Preference weight scheme (``"grade_diff"`` or ``"binary"`` for qrels;
        ignored when preference_source is ``"scores"``).
    preference_source:
        ``"qrels"`` (from relevance labels) or ``"scores"`` (from scores;
        uses relevance as proxy when no scorer is provided).
    score_weight_scheme:
        When preference_source is ``"scores"``: ``"binary"``, ``"absolute_margin"``,
        or ``"normalized_margin"``.
    scores_by_query:
        When set and preference_source is ``"scores"``, use real model scores.
        Mapping query_id → [(doc_id, score), ...]. If None or query missing,
        fall back to relevance-as-proxy.
    multi_scores_by_query:
        When set and preference_source is ``"multi_scores"``, use aggregated
        rankings. Mapping query_id → {scorer_name: [(doc_id, score), ...]}.
    multi_score_weight_mode:
        Aggregation mode for multi_scores: majority_vote, summed_margin, vote_plus_margin.
    seed:
    ...
    # ------------------------------------------------------------------
    # Safeguard: skip if too few relevance-judged documents
    # ------------------------------------------------------------------
    unique_docs = {e.doc_id for e in qrels_for_query}
    n_distinct_grades = len({e.relevance for e in qrels_for_query})
    # When using scores or multi_scores, we only need >=1 judged doc (for evaluation)
    min_docs = 1 if preference_source in ("scores", "multi_scores") else MIN_JUDGED_DOCS
    need_distinct_grades = preference_source == "qrels"
    if len(unique_docs) < min_docs:
        return [], {
            "query_id": qid,
            "reason": f"only {len(unique_docs)} judged docs (need >= {min_docs})",
        }
    if need_distinct_grades and n_distinct_grades < 2:
        return [], {
            "query_id": qid,
            "reason": f"only {n_distinct_grades} distinct grade(s) (need >= 2 for qrels)",
        }
    ...
    with Timer("pairwise_preference_generation", accumulator=query_acc):
        if preference_source == "qrels":
            ...
            scorer_rankings = multi_scores_by_query[qid]
            # Truncate each scorer's list to top_k
            scorer_rankings_truncated = {
                name: cands[:top_k] for name, cands in scorer_rankings.items()
            }
            schema_prefs = preferences_from_multiple_score_rankings(
                query_id=qid,
                scorer_rankings=scorer_rankings_truncated,
                weight_mode=multi_score_weight_mode,
            )
        else:  # preference_source == "scores"
            # Use real model scores if available, else relevance as proxy
            if scores_by_query and qid in scores_by_query:
                candidates = scores_by_query[qid][:top_k]
            else:
                if scores_by_query is not None:
                    log.debug("Query %s not in scores file; using relevance as proxy", qid)
                sorted_entries = sorted(
                    qrels_for_query, key=lambda e: e.relevance, reverse=True
                )
                candidates = [(e.doc_id, float(e.relevance)) for e in sorted_entries[:top_k]]
            schema_prefs = preferences_from_scores(
                query_id=qid,
    ...
    # ------------------------------------------------------------------
    # 4. Reference ranking and relevance for retrieval metrics
    # ------------------------------------------------------------------
    ref_ranking = _reference_ranking(qrels_for_query[:top_k])
    relevance_map = {e.doc_id: e.relevance for e in qrels_for_query}
    relevant_ids = {e.doc_id for e in qrels_for_query if e.relevance > 0}
    ...
    # Raw score ranking - use first available scorer when scores or multi_scores
    if preference_source == "scores" and scores_by_query and qid in scores_by_query:
        raw_ranking = [doc_id for doc_id, _ in scores_by_query[qid][:top_k]]
        rankings["raw_score"] = raw_ranking
    elif preference_source == "multi_scores" and multi_scores_by_query and qid in multi_scores_by_query:
        # Use first scorer's order as raw baseline
        first_scorer = next(iter(multi_scores_by_query[qid].values()), [])
        raw_ranking = [doc_id for doc_id, _ in first_scorer[:top_k]]
        rankings["raw_score"] = raw_ranking
    ...
    with Timer("evaluation", accumulator=query_acc):
        ...
        for method_name in METHODS:
            ...
            ndcg10 = ndcg_at_k(ranking, relevance_map, k=10)
            rr = mrr(ranking, relevant_ids)
            r10 = recall_at_k(ranking, relevant_ids, k=10)
            r20 = recall_at_k(ranking, relevant_ids, k=20)
    ...
    for method_name in METHODS:
        ...
        rows.append({
            ...
            "ndcg_at_10": round(m_metrics["ndcg_at_10"], 6),
            "mrr": round(m_metrics["mrr"], 6),
            "recall_at_10": round(m_metrics["recall_at_10"], 6),
            "recall_at_20": round(m_metrics["recall_at_20"], 6),
            # Timings
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="qrels",
        choices=["qrels", "scores", "multi_scores"],
        help="Source of pairwise preferences: qrels, scores, or multi_scores (aggregate multiple scorers).",
    )
    parser.add_argument(
        "--weight-scheme",
        type=str,
        ...
        help="Preference weight scheme when --preference-source=qrels.",
    )
    parser.add_argument(
        "--score-weight-scheme",
        type=str,
        default="absolute_margin",
        choices=["binary", "absolute_margin", "normalized_margin"],
        help="Weight scheme when --preference-source=scores.",
    )
    parser.add_argument(
        "--scores-file",
        type=Path,
        default=None,
        help="Path to JSONL file with per-query scores (query_id, ranked_doc_ids, scores). "
        "When set with --preference-source=scores, uses real model scores.",
    )
    parser.add_argument(
        "--scorer",
        type=str,
        default=None,
        help="Shorthand: use data/processed/<dataset>/scores/<scorer>.jsonl. "
        "Ignored if --scores-file or --preference-source=multi_scores.",
    )
    parser.add_argument(
        "--scorers",
        type=str,
        default=None,
        help="Comma-separated scorer names for --preference-source=multi_scores. "
        "Looks for data/processed/<dataset>/scores/<name>.jsonl per scorer.",
    )
    parser.add_argument(
...
def run_experiment(
    ...
    seed: int,
    output_dir: Path,
    preference_source: str = "qrels",
    score_weight_scheme: str = "absolute_margin",
    scores_file: Path | None = None,
    scorer: str | None = None,
    scorers: str | None = None,
    multi_score_weight_mode: str = "majority_vote",
    ...
    print(f"  top_k              : {top_k}")
    print(f"  preference_source  : {preference_source}")
    print(f"  weight_scheme      : {weight_scheme}")
    print(f"  score_weight_scheme: {score_weight_scheme}")
    print(f"  seed               : {seed}")
    print(f"  output_dir         : {output_dir}")
    print(f"  save_timings       : {save_timings}")
    if scores_file is not None or scorer is not None:
        print(f"  scores_file        : {scores_file or 'N/A (using --scorer)'}")
        print(f"  scorer            : {scorer or 'N/A'}")
    if preference_source == "multi_scores":
        print(f"  scorers            : {scorers or 'N/A'}")
        print(f"  multi_score_weight_mode: {multi_score_weight_mode}")
    print()
    ...
    # ------------------------------------------------------------------
    # 1b. Load score rankings (if requested)
    # ------------------------------------------------------------------
    scores_by_query: dict[str, list[tuple[str, float]]] | None = None
    multi_scores_by_query: dict[str, dict[str, list[tuple[str, float]]]] | None = None

    if preference_source == "multi_scores":
        if not scorers:
            log.error("--preference-source=multi_scores requires --scorers (e.g. --scorers bm25,dense)")
            sys.exit(1)
        ...
        with Timer("score_loading", accumulator=global_acc):
            loaded = load_multi_scorer_rankings(scorer_paths)
        if len(loaded) < 2:
            log.error(
                "multi_scores requires at least 2 scorers. Found %d. "
                "Ensure score files exist at: %s",
                len(loaded),
                [str(p) for p in scorer_paths.values()],
            )
            sys.exit(1)
        ...
        print(
            f"[1b] Loaded {len(loaded)} scorers; {len(multi_scores_by_query)} queries "
            f"with >=2 scorers"
        )
        ...
        if not scores_path.exists():
            ...
            sys.exit(1)
        with Timer("score_loading", accumulator=global_acc):
            scores_by_query = load_score_rankings(scores_path)
        print(f"[1b] Loaded scores for {len(scores_by_query)} queries from {scores_path}")
    elif scorer is not None:
        cfg = get_config(dataset)
        scores_path = cfg.processed_path / "scores" / f"{scorer}.jsonl"
        if not scores_path.exists():
            log.error("Scorer file not found: %s", scores_path)
            sys.exit(1)
        with Timer("score_loading", accumulator=global_acc):
            scores_by_query = load_score_rankings(scores_path)
        print(f"[1b] Loaded scores for {len(scores_by_query)} queries from {scores_path}")
    ...
    with Timer("candidate_selection", accumulator=global_acc):
        ...
        # Restrict to queries that have enough qrel entries (for evaluation)
        min_entries = 1 if (scores_by_query is not None or multi_scores_by_query is not None) else MIN_JUDGED_DOCS
        eligible_qids = sorted(
            qid for qid, entries in qrels_by_query.items() if len(entries) >= min_entries
        )
        # When using scores, also require query to be in score file
        if scores_by_query is not None:
            eligible_qids = [qid for qid in eligible_qids if qid in scores_by_query]
        if multi_scores_by_query is not None:
            eligible_qids = [qid for qid in eligible_qids if qid in multi_scores_by_query]
    ...
    with Timer("total_pipeline", accumulator=global_acc):
        ...
        for idx, qid in enumerate(sampled_qids):
            ...
            rows, skip_info = run_query(
                ...
                score_weight_scheme=score_weight_scheme,
...
def _build_summary(rows: list[dict]) -> list[dict]:
    ...
    for method in METHODS:
        ...
        ndcg10 = _stats("ndcg_at_10")
        mrr_s = _stats("mrr")
        r10 = _stats("recall_at_10")
        r20 = _stats("recall_at_20")
        ...
        summary_rows.append({
            ...
            "tau_max": tau["max"],
            # Retrieval metrics
            "ndcg_at_10_mean": ndcg10["mean"],
            "mrr_mean": mrr_s["mean"],
            "recall_at_10_mean": r10["mean"],
            "recall_at_20_mean": r20["mean"],
            # Runtime
...
def _build_experiment_summary(
    ...
    # Retrieval metrics by method (higher is better)
    def _avg_by_method(key: str) -> dict[str, float]:
        by_m: dict[str, list[float]] = defaultdict(list)
        for r in all_rows:
            if r.get(key) is not None:
                by_m[r["method"]].append(r[key])
        return {m: sum(vs) / len(vs) for m, vs in by_m.items() if vs}

    avg_ndcg10 = _avg_by_method("ndcg_at_10")
    avg_mrr = _avg_by_method("mrr")
    avg_r10 = _avg_by_method("recall_at_10")
    avg_r20 = _avg_by_method("recall_at_20")
    ...
    return {
        ...
        "avg_ndcg_at_10_by_method": {m: round(v, 6) for m, v in avg_ndcg10.items()},
        "avg_mrr_by_method": {m: round(v, 6) for m, v in avg_mrr.items()},
        "avg_recall_at_10_by_method": {m: round(v, 6) for m, v in avg_r10.items()},
        "avg_recall_at_20_by_method": {m: round(v, 6) for m, v in avg_r20.items()},
        "global_timings": {
...
def _print_experiment_summary(summary: dict, dataset: str) -> None:
    ...
    for m, v in summary["avg_bew_by_method"].items():
        print(f"    {m:<30} {v:.4f}")
    print(f"\n  Retrieval metrics (mean by method):")
    for method in METHODS:
        ndcg = summary.get("avg_ndcg_at_10_by_method", {}).get(method, 0)
        mrr = summary.get("avg_mrr_by_method", {}).get(method, 0)
        r10 = summary.get("avg_recall_at_10_by_method", {}).get(method, 0)
        r20 = summary.get("avg_recall_at_20_by_method", {}).get(method, 0)
        print(f"    {method:<30} NDCG@10={ndcg:.4f}  MRR={mrr:.4f}  R@10={r10:.4f}  R@20={r20:.4f}")
...
def main(argv: list[str] | None = None) -> None:
    ...
    run_experiment(
        ...
        score_weight_scheme=args.score_weight_scheme,
