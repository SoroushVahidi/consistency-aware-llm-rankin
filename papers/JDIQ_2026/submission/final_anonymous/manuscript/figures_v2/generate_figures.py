...
SUPERSEDED for Figures 3 and 5 as of 2026-07-14: main.tex now includes
manually-prepared figure3.png/figure5.png (uploaded directly to the
manuscript directory) instead of this script's fig3_cyclicity_primary.pdf
and fig5_cycle_decomposition.pdf outputs. fig3_cyclicity_primary() and
fig5_cycle_decomposition() below are left intact (not deleted/disabled) as
a fallback vector source and are still correct against the same canonical
CSVs, but running them no longer affects the compiled manuscript unless
main.tex's \\includegraphics paths for Figures 3 and 5 are changed back.
"""
...
def fig9_sign_change_heatmap():
    ...
    fig.legend(loc="upper center", bbox_to_anchor=(0.5, 1.1), ncol=2, frameon=False, fontsize=BASE_SIZE)
    fig.tight_layout()
...
def fig10_baseline_comparison():
    ...
    fig.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, 1.12), ncol=2, frameon=False, fontsize=BASE_SIZE)
    fig.tight_layout()
