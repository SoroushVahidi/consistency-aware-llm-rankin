...
\begin{table}[t]
  ...
  \small
  \begin{tabular}{p{2.3cm}p{1.7cm}p{1.9cm}p{2.4cm}}
    \toprule
    Stream & Mode / model & Scope & Role \\
    \midrule
    OpenAI API & Pairwise, \texttt{gpt-4o-mini} & SciDocs 50, HotpotQA 20, FiQA 10 usable of 20 target queries & Primary paired pilot \\
    OpenAI API & Pointwise / listwise, \texttt{gpt-4o-mini} & SciDocs 20+20, HotpotQA 10+10 queries & Auxiliary prompt-style scope check \\
    Cohere compatibility API & Pairwise, \texttt{command-r-plus-08-2024} & FiQA, HotpotQA, BRIGHT; 200 query$\times$regime records & Protocol-distinct corroborative corpus \\
    Azure OpenAI v1 API & Pairwise, \texttt{gpt-4.1-mini} & FiQA, HotpotQA, BRIGHT; 200 query$\times$regime records & Protocol-distinct corroborative corpus \\
    \bottomrule
  \end{tabular}
\end{table}
